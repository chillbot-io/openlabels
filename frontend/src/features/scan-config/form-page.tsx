...
import { ScheduleFields, scheduleFieldsSchema } from '@/components/forms/schedule-fields.tsx';
import { useUIStore } from '@/stores/ui-store.ts';

const scanConfigSchema = scheduleFieldsSchema.extend({
  exclude_paths: z.string(),
  exclude_file_types: z.string(),
});

type FormData = z.infer<typeof scanConfigSchema>;
  ...
  const onSubmit = (data: FormData) => {
    const payload = {
      ...
      exclude_paths: data.exclude_paths
        ? data.exclude_paths.split(',').map((p) => p.trim()).filter(Boolean)
        : [],
      exclude_file_types: data.exclude_file_types
        ? data.exclude_file_types.split(',').map((t) => t.trim()).filter(Boolean)
        : [],
    };

...
Per Reference Architecture v2.3, Stage 4:
...
# Pronouns to expand (per architecture spec)
PRONOUNS = frozenset([
    "he", "she", "they",
    "him", "her", "them",
...
def _get_models_dir() -> Path:
    """Get models directory, using config default if not explicitly set."""
    global _MODELS_DIR
...
def _get_onnx_session():
    ...
    with _INIT_LOCK:
        ...
        # Prefer CPU for consistency (CUDA optional)
...
def _get_tokenizer():
    ...
    with _INIT_LOCK:
        ...
        if tokenizer_json_path.exists():
            ...
            # NO PADDING - dynamic length for fast inference
...
def _tokenize_for_coref(text: str) -> Tuple[np.ndarray, np.ndarray, List[Tuple[int, int]]]:
    ...
    if is_fast:
        # tokenizers library - no padding, dynamic length
        encoded = tokenizer.encode(text)
        input_ids = np.array([encoded.ids], dtype=np.int64)
        attention_mask = np.array([encoded.attention_mask], dtype=np.int64)
        offset_mapping = encoded.offsets
    else:
        # transformers library - no padding
        inputs = tokenizer(
            text,
            return_tensors="np",
...
def _get_mention_candidates(
    ...
    Uses heap-based selection for O(n * max_span_length * log(k)) complexity
    instead of O(n²) from building full candidate list + sorting.

    Returns list of (start_idx, end_idx, score) tuples, sorted by score descending.
    """
    valid_len = int(attention_mask.sum())

    # Use a min-heap of size top_k for efficient top-k selection
    # Heap elements: (score, start, end) - score first for heap ordering
    heap: List[Tuple[float, int, int]] = []

    # Consider all valid (start, end) pairs
    ...
    # Extract and convert to (start, end, score) format, sorted by score descending
...
def _compute_antecedent_scores(
    ...
    Returns [num_mentions, num_mentions] score matrix.
    """
    n = len(mentions)
    if n == 0:
        return np.zeros((0, 0))
...
def _cluster_mentions(
    ...
    Returns list of clusters, each cluster is list of (start, end) spans.
    """
    n = len(mentions)
    if n == 0:
        return []
    ...
    for cid in sorted(clusters.keys()):
        ...
        if len(cluster_spans) > 1:  # Only include clusters with multiple mentions
...
def _resolve_with_onnx(
    ...
    Algorithm:
    1. Run ONNX model to get mention scores and embeddings
    2. Extract top-k mention candidates
    3. Compute pairwise antecedent scores
    4. Cluster mentions greedily
    5. Find clusters containing our NAME anchors
    6. Expand to pronouns in those clusters
    """
    ...
    # Unpack outputs (batch dim = 0)
    mention_start_scores = outputs[0][0]  # [seq]
    mention_end_scores = outputs[1][0]    # [seq]
    mention_s2e = outputs[2][0]           # [seq, 1024]
    end_mention = outputs[3][0]           # [seq, 1024]
    ante_s2s = outputs[4][0]              # [seq, 1024]
    ante_e2e = outputs[5][0]              # [seq, 1024]
    ante_s2e = outputs[6][0]              # [seq, 1024]
    ante_e2s = outputs[7][0]              # [seq, 1024]
    ...
    def find_overlapping_anchor(start: int, end: int) -> Optional[Span]:
        """Find anchor that overlaps with given span."""
    ...
    return result
# RULE-BASED FALLBACK
# Gender heuristics for rule-based matching
    ...
    for match in pronoun_matches:
        ...
        for anchor, gender in anchors:
            ...
            if abs(pronoun_sent_idx - anchor_sent_idx) > window_sentences:
                continue
            # Skip if pronoun overlaps with anchor (using half-open intervals)
            # pstart < anchor.end means actual overlap; pstart == anchor.end is adjacent (OK)
        ...
        new_span = Span(
            ...
            tier=Tier.ML,
            coref_anchor_value=anchor.text,  # Link to anchor for same token
        )
...
def _link_partial_names(spans: List[Span], min_confidence: float = 0.70) -> List[Span]:
    ...
    When "Smith" appears after "John Smith" was detected, set
    coref_anchor_value="John Smith" so tokenizer assigns same token.
    
    Rules:
    - Only NAME-family types (NAME, NAME_PATIENT, NAME_PROVIDER, NAME_RELATIVE)
    - Longer name becomes anchor, shorter names link to it
    - Names are grouped by word overlap (e.g., "Smith" overlaps with "John Smith")
    - Don't overwrite existing coref_anchor_value (from pronoun resolution)
    - No position/window constraints (partial names can link across entire document)
    
    Args:
        spans: Spans with NAME-type detections
        min_confidence: Minimum confidence to consider as anchor
    
    Returns:
        Spans with coref_anchor_value set for partial name matches
    """
    if not spans:
        return []
    
    # Collect NAME-type spans eligible for linking
    name_spans: List[Tuple[int, Span]] = []  # (index, span)
    for i, span in enumerate(spans):
        if span.entity_type in NAME_TYPES and span.confidence >= min_confidence:
            name_spans.append((i, span))
    
    if len(name_spans) < 2:
        # Need at least 2 names to link
        return list(spans)
    
    # Build word-to-spans index for grouping
    # Maps each word to list of (span_index, span) that contain it
    word_to_spans: Dict[str, List[Tuple[int, Span]]] = {}
    span_words: Dict[int, Set[str]] = {}  # span_index -> words
    ...
    # For each group, find anchor (longest name) and link others to it
    result = list(spans)
    ...
    for group_id, member_indices in groups.items():
        ...
        # Find anchor: longest text, then earliest position as tiebreaker
        ...
        for idx, span in members:
            ...
            # Create new span with coref_anchor_value set
            logger.debug(
...
def resolve_coreferences(
    ...
    Per Reference Architecture v2.3:
    - Pronouns (he/she/they/etc.) linked to NAME anchors become spans
    - Partial names (e.g., "Smith" after "John Smith") linked to full names
    - New spans inherit entity_type from anchor
    - New span confidence = anchor.confidence × 0.90
    ...
    # Step 1: Pronoun resolution (adds new spans for he/she/they/etc.)
    if use_onnx:
        try:
    ...
    # Step 2: Partial name linking (links "Smith" to "John Smith", etc.)
    # This sets coref_anchor_value so tokenizer assigns same token
    result = _link_partial_names(result, min_confidence=min_anchor_confidence)

    # Step 3: Validate span positions
    # Filters any invalid spans created by coref (defensive measure)

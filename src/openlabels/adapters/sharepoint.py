...
class SharePointAdapter:
    ...
    async def _get_token(self) -> str:
        ...
        async with httpx.AsyncClient() as client:
            ...
            self._token_expires = datetime.utcnow()  # Simplified; should parse expires_in
    ...
    async def list_sites(self) -> list[dict]:
        ...
        data = await self._graph_get("/sites?search=*")
    ...
    async def list_files(
        ...
        """
        # If target is a URL, resolve to site ID
        site_id = target
        if target.startswith("https://"):
            site_info = await self._graph_get(f"/sites/{target.replace('https://', '')}")
            site_id = site_info["id"]

        # Get root drive
        drive = await self._graph_get(f"/sites/{site_id}/drive")
        drive_id = drive["id"]

        # List files recursively
        async for file_info in self._list_drive_items(site_id, drive_id, "/", recursive):
            yield file_info
            ...
            if "folder" in item:
                if recursive:
                    ...
                    async for file_info in self._list_drive_items(
                        site_id, drive_id, folder_path, recursive
                    ):
                        yield file_info
            elif "file" in item:
                yield self._item_to_file_info(item, site_id)
    ...
    async def get_metadata(self, file_info: FileInfo) -> FileInfo:
        ...
        return self._item_to_file_info(item, file_info.site_id)

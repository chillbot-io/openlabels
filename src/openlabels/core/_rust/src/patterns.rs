//! Built-in pattern definitions for common sensitive data types.
//!
//! Each pattern is defined as (name, regex, validator, base_confidence).

/// Built-in patterns for sensitive data detection.
/// Format: (name, regex_pattern, validator, base_confidence)
pub static BUILTIN_PATTERNS: &[(&str, &str, Option<&str>, f64)] = &[
    // === FINANCIAL ===
    ...
    (
        "US_BANK_ROUTING",
        r"\b(?:0[1-9]|1[0-2]|2[1-9]|3[0-2]|6[1-9]|7[0-2]|80)[0-9]{7}\b",
        None,
        0.60,
    ),
    ...
    (
        "SEDOL",
        r"\b[0-9BCDFGHJKLMNPQRSTVWXYZ]{7}\b",
        None,
        0.65,
    ),
    ...
    // National Provider Identifier (Healthcare)
    (
        "NPI",
        r"\b[12][0-9]{9}\b",
        Some("npi"),
    ...
    (
        "DEA_NUMBER",
        r"\b[ABCDEFGHJKLMNPRSTUVWXYabcdefghjklmnprstuvwxy][A-Za-z9][0-9]{7}\b",
        None,
        0.75,
    ),
    ...
    (
        ...
        r"(?i)\b(?:TOP\s*SECRET|SECRET|CONFIDENTIAL|UNCLASSIFIED|FOUO|NOFORN|ORCON|REL\s*TO)\b",
        None,
        0.95,
    ),
];
